...
def test_single_click_on_directory_expands_it_once_panel_is_focused():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fake = FakePanel(tmp)
        fake._handle_worktree_click(0)  # focuses the panel (header row)
        assert fake.worktree_focused is True
        entries = fake._worktree_entries()
        dir_row = 1 + next(
            i for i, e in enumerate(entries) if e[1] == "sub"
        )
...
def test_single_click_on_file_still_only_selects():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fake = FakePanel(tmp)
        fake._handle_worktree_click(0)  # focuses; cursor defaults to 0 (a.txt)
